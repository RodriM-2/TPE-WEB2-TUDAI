<?php
    ...
    class ItemModel extends modelDB {

        protected $table='gatos';

        public function GetItem($id) {
            $sentence= $this->db->prepare("SELECT * FROM {$this->table} WHERE id=?");
            $sentence->execute([$id]);
        ...
        public function EditItem($nombre,$edad,$raza,$color,$peso,$observaciones,$id) {
            $sentence= $this->db->prepare("UPDATE {$this->table} SET nombre=?, edad_meses=?, raza,color=?, peso_kg=?, observaciones=? WHERE id=?");
